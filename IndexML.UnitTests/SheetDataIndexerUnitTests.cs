namespace IndexML.UnitTests
{
    ...
    /// <summary>
    /// Unit tests for the <see cref="SheetDataIndexer"/> class.
    /// </summary>
    [TestClass]
    public class SheetDataIndexerUnitTests : OpenXmlIndexerTest
    {
        ...
        {
            var target = new SheetDataIndexer(null);
        }
        ...
        public void Constructor_EmptySheetData_ValidEmptyState()
        {
            SafeExecuteTest(
                ...
                (sheetData) =>
                {
                    var target = new SheetDataIndexer(sheetData);
        ...
        public void Constructor_MaxExtents_ValidState()
        {
            SafeExecuteTest(
                ...
                (sheetData) =>
                {
                    ...
                    Assert.AreEqual(SheetDataIndexer.Capacity, target.MaxRowIndex);
                    ValidateRowSequence(target);
                });
        }
        ...
        public void RemoveRow_OverCapacityIndex_ThrowsException()
        {
            SafeExecuteTest(
                ...
                {
                    var target = new SheetDataIndexer(sheetData);
                    target.RemoveRow(SheetDataIndexer.Capacity + 2); // Row indices are one based, hence the +2 is needed
                });
        }
        ...
        public void InsertRow_IndexOverCapacity_ThrowsException()
        {
            SafeExecuteTest(
                ...
                {
                    var target = new SheetDataIndexer(sheetData);
                    target.InsertRow(new Row(), SheetDataIndexer.Capacity + 1);
                });
        }
